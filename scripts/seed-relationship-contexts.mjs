/**
 * Bulk-create Relationship Context records for a coach's clients.
 *
 * Usage:
 *   SEED_COACH_NAME="Jane Doe" DRY_RUN=true node scripts/seed-relationship-contexts.mjs   # preview only
 *   SEED_COACH_NAME="Jane Doe" node scripts/seed-relationship-contexts.mjs                 # create records
 *
 * SEED_COACH_NAME is matched (case-insensitively) against each user's name; the
 * matched user must also have a coach/admin Role. Requires AIRTABLE_API_KEY and
 * AIRTABLE_BASE_ID from .env.local.
 */
...
const COACH_NAME = (process.env.SEED_COACH_NAME ?? '').trim()
...
if (!COACH_NAME) {
  console.error('Missing SEED_COACH_NAME (e.g. SEED_COACH_NAME="Jane Doe")')
  process.exit(1)
}
...
async function main() {
  ...
  // 2. Find the coach by name (must also be a coach/admin)
  const coachNeedle = COACH_NAME.toLowerCase()
  const coachRecord = allUsers.find((r) => {
    const f = r.fields
    const name = nameOf(f).toLowerCase()
    const role = (f['Role'] ?? '').toLowerCase()
    return name.includes(coachNeedle) && (role === 'coach' || role === 'admin')
  })

  if (!coachRecord) {
    console.error(`Could not find a coach/admin named "${COACH_NAME}" in the Users table`)
    process.exit(1)
  }

  const coachId = coachRecord.id
  const coachName = nameOf(coachRecord.fields)
  console.log(`  Coach: ${coachName} (${coachId})`)

  // 3. Find the coach's clients — users whose Coach linked field includes the coach's record ID
  const coachClients = allUsers.filter((r) => {
    const coachIds = Array.isArray(r.fields['Coach']) ? r.fields['Coach'] : []
    return coachIds.includes(coachId)
  })

  console.log(`  ${coachName}'s clients: ${coachClients.length}`)
  ...
  for (const client of coachClients) {
    ...
    const key = `${client.id}|${coachId}|executive coaching`
  ...
  for (const r of toCreate) {
    console.log(`  + ${r.personName} (${r.personId}) coached by ${coachName}`)
  }
